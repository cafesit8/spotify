    ...
    <>
      <button onClick={() => setOpen(!open)} type='button' className='h-full flex-1 p-0 overflow-hidden'>
        <article className='flex h-full items-center gap-2 overflow-hidden'>
          ...
          <div>
            <p className='font-light text-start text-white lg:text-base text-sm truncate'>{currentSong?.name || 'Título de la canción'}</p>
            <p className='font-light text-start text-white/80 lg:text-sm text-xs truncate'>{currentSong?.artist || 'Nombre del artista'}</p>
          </div>
        </article>
      </button>
      <button aria-label={`${playing ? 'Pausar Canción' : 'Reproducir Canción'}`} onClick={() => setPlaying(!playing)} className='h-full w-14 grid place-content-center'>
