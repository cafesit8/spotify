...
export default function PlayList ({ url, subtitle }: { url: string, subtitle: string }) {
  ...
  return (
    <section ref={ref} className='flex flex-col gap-2'>
      ...
      {playList.length > 0
        ? <div className="grid lg:gap-3 gap-2 lg:[grid-template-columns:repeat(auto-fill,minmax(160px,1fr))] sm:[grid-template-columns:repeat(auto-fill,minmax(120px,1fr))] [grid-template-columns:repeat(auto-fill,minmax(100px,1fr))]">
          {playList?.map(song => (
